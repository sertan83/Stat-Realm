...
export async function loadMyReviewsPage(input: {
  steamId: string;
  page?: number;
  ...
  const reviewsWithText = storedRatings.filter(
    (rating) => rating.reviewText && rating.reviewText.trim().length > 0,
  );

  const sortedRatings = [...reviewsWithText].sort(
  ...
  const reviewEntries = sortedRatings.map((rating) => {
